...
| HTTPS 回调 | 入口与执行时均校验 HTTPS，正常验证证书，拒绝跳转、URL 账号密码及内网连接 |
...
只支持有效证书的公网 HTTPS 回调。禁止跳转和连接内网、回环、链路本地地址；DNS 解析后的 IP 在实际连接前校验，避免 DNS 重绑定。IP 地址形式的 HTTPS URL 仍需证书包含该 IP。测试用的自签名证书不能通过关闭生产证书校验来兼容。
...
回滚旧二进制不需要删除新增表，但旧版本不认识新的回执队列：需先排空或保留任务，修复后恢复新版本处理。上游提交结果不确定时保持 sending/uncertain，不自动重放短信。memory driver 重启丢失全部内存状态，生产可靠性必须使用 PostgreSQL，或单进程持久化 file driver。
